...
class AzadaWholesaler extends ObjectModel
{
    ...
    {
        self::ensureDatabaseStructure();
        $daysToDelete = AzadaConfig::getInt('AZADA_B2B_DELETE_DAYS', 7);
        if ($daysToDelete > 0) {
        ...
        if ($allInvoices) {
            foreach ($allInvoices as $inv) {
                ...
                if (!empty($inv['file_name'])) {
                    $ok = AzadaDbRepository::ensureInvoiceFileInFV($inv['file_name']);
                }
                if (!$ok && !AzadaDbRepository::invoiceFileExistsAnyCase($inv['file_name'])) {
                    AzadaDbRepository::deleteInvoiceById($inv['id_invoice']);
                }
            }
        }
    ...
    public static function processInvoiceDownload($wholesalerId, $docNumber, $docDate, $docNetto, $docDeadline, $isPaid, $downloadUrl, $cookieFile = null)
{
    ...
    public static function ensureDatabaseStructure()
    {
        ...
        $db->execute("CREATE TABLE IF NOT EXISTS `$tableHubSettings` (
            ...
            `seo_strip_style` TINYINT(1) NOT NULL DEFAULT 1,
            `seo_strip_iframe` TINYINT(1) NOT NULL DEFAULT 1,
            `seo_strip_links` TINYINT(1) NOT NULL DEFAULT 0,
            `seo_short_desc_fallback` TINYINT(1) NOT NULL DEFAULT 1,
            `seo_meta_title_template` VARCHAR(255) DEFAULT NULL,
            `seo_meta_desc_template` VARCHAR(255) DEFAULT NULL,
            `seo_description_prefix` VARCHAR(255) DEFAULT NULL,
            `seo_description_suffix` VARCHAR(255) DEFAULT NULL,
            `quality_require_ean` TINYINT(1) NOT NULL DEFAULT 1,
            `quality_require_name` TINYINT(1) NOT NULL DEFAULT 1,
            `quality_require_price` TINYINT(1) NOT NULL DEFAULT 1,
            `quality_require_stock` TINYINT(1) NOT NULL DEFAULT 1,
            `quality_reject_missing_data` TINYINT(1) NOT NULL DEFAULT 1,
        ...
        $hubColumns = [
            ...
            'seo_strip_style' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_strip_style` TINYINT(1) NOT NULL DEFAULT 1 AFTER `zero_below_stock`",
            'seo_strip_iframe' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_strip_iframe` TINYINT(1) NOT NULL DEFAULT 1 AFTER `seo_strip_style`",
            'seo_strip_links' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_strip_links` TINYINT(1) NOT NULL DEFAULT 0 AFTER `seo_strip_iframe`",
            'seo_short_desc_fallback' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_short_desc_fallback` TINYINT(1) NOT NULL DEFAULT 1 AFTER `seo_strip_links`",
            'seo_meta_title_template' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_meta_title_template` VARCHAR(255) DEFAULT NULL AFTER `seo_short_desc_fallback`",
            'seo_meta_desc_template' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_meta_desc_template` VARCHAR(255) DEFAULT NULL AFTER `seo_meta_title_template`",
            'seo_description_prefix' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_description_prefix` VARCHAR(255) DEFAULT NULL AFTER `seo_meta_desc_template`",
            'seo_description_suffix' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `seo_description_suffix` VARCHAR(255) DEFAULT NULL AFTER `seo_description_prefix`",
            'quality_require_ean' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `quality_require_ean` TINYINT(1) NOT NULL DEFAULT 1 AFTER `seo_description_suffix`",
            'quality_require_name' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `quality_require_name` TINYINT(1) NOT NULL DEFAULT 1 AFTER `quality_require_ean`",
            'quality_require_price' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `quality_require_price` TINYINT(1) NOT NULL DEFAULT 1 AFTER `quality_require_name`",
            'quality_require_stock' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `quality_require_stock` TINYINT(1) NOT NULL DEFAULT 1 AFTER `quality_require_price`",
            'quality_reject_missing_data' => "ALTER TABLE `$tableHubSettings` ADD COLUMN `quality_reject_missing_data` TINYINT(1) NOT NULL DEFAULT 1 AFTER `quality_require_stock`",
        ];
        ...
        if (is_array($allWholesalers)) {
            foreach ($allWholesalers as $wholesalerRow) {
                ...
                $db->insert('azada_wholesaler_pro_hub_settings', [
                    ...
                    'seo_strip_iframe' => 1,
                    'seo_strip_links' => 0,
                    'seo_short_desc_fallback' => 1,
                    'seo_meta_title_template' => null,
                    'seo_meta_desc_template' => null,
                    'seo_description_prefix' => null,
                    'seo_description_suffix' => null,
                    'quality_require_ean' => 1,
                    'quality_require_name' => 1,
                    'quality_require_price' => 1,
                    'quality_require_stock' => 1,
                    'quality_reject_missing_data' => 1,

...
class AzadaAbro
{
    ...
    /**
     * LIGHT import dla ABRO – szybka synchronizacja STANÓW (i tylko stanów) do tabeli docelowej `azada_raw_abro`.
     *
     * Cel: możliwość odświeżania ilości co kilka minut bez DROP/CREATE i bez przebudowy całej tabeli.
     *
     * Mechanizm:
     * - pobiera XML z ABRO,
     * - wyciąga EAN + stock + multiplier,
     * - przelicza docelowy stan (opakowania jeśli multiplier>0, inaczej szt.),
     * - ładuje do temp tabeli,
     * - robi UPDATE JOIN na `azada_raw_abro`.
     *
     * Uwaga: jeżeli tabela docelowa nie istnieje (np. pierwszy start) → fallback do FULL import.
     */
    public static function importProductsLight($wholesaler)
    {
        $targetTableName = _DB_PREFIX_ . 'azada_raw_abro';

        // Jeżeli nie ma tabeli docelowej, robimy pełny import (żeby zbudować bazę danych)
        $exists = (bool)Db::getInstance()->getValue(
            "SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '" . pSQL($targetTableName) . "'"
        );
        if (!$exists) {
            return self::importProducts($wholesaler);
        }

        // Upewniamy się, że kolumny rozszerzone istnieją (nie jest to krytyczne dla stanów,
        // ale pozwala uzupełnić pola audytowe używane w module).
        self::ensureTargetExtraColumns($targetTableName);

        $feed = self::downloadFeed($wholesaler->api_key);
        if (empty($feed['status'])) {
            return ['status' => 'error', 'msg' => isset($feed['msg']) ? $feed['msg'] : 'Nie udało się pobrać XML ABRO (LIGHT).'];
        }

        $xmlContent = (string)$feed['content'];
        if ($xmlContent === '') {
            return ['status' => 'error', 'msg' => 'Pusty XML ABRO (LIGHT).'];
        }

        // Temp tabela na stany (po EAN)
        $tmpName = _DB_PREFIX_ . 'azada_tmp_abro_stock';
        $db = Db::getInstance();

        // Tworzymy jako TEMPORARY – zniknie po zakończeniu połączenia
        $db->execute("DROP TEMPORARY TABLE IF EXISTS `" . bqSQL($tmpName) . "`");
        $create = "CREATE TEMPORARY TABLE `" . bqSQL($tmpName) . "` (
            `kod_kreskowy` varchar(64) NOT NULL,
            `target_stock` int(11) NOT NULL DEFAULT 0,
            `raw_stock_szt` decimal(20,6) NOT NULL DEFAULT 0.000000,
            `pack_qty` int(11) NOT NULL DEFAULT 0,
            `conversion_applied` tinyint(1) NOT NULL DEFAULT 0,
            PRIMARY KEY (`kod_kreskowy`)
        ) ENGINE=" . _MYSQL_ENGINE_ . " DEFAULT CHARSET=utf8;";

        if (!$db->execute($create)) {
            return ['status' => 'error', 'msg' => 'Nie udało się utworzyć tabeli tymczasowej dla ABRO (LIGHT).'];
        }

        $insertBase = "INSERT INTO `" . bqSQL($tmpName) . "` (`kod_kreskowy`,`target_stock`,`raw_stock_szt`,`pack_qty`,`conversion_applied`) VALUES ";
        $batch = [];
        $seen = 0; // ile rekordów (EAN) wstawiono do tabeli tymczasowej
        $offersSeen = 0; // ile węzłów <o> znaleziono w XML (debug)
        $now = date('Y-m-d H:i:s');

        // Stream parsing dla wydajności
        $reader = new XMLReader();
        libxml_use_internal_errors(true);
        $ok = $reader->XML($xmlContent, 'UTF-8', LIBXML_NOERROR | LIBXML_NOWARNING | LIBXML_NONET);
        if (!$ok) {
            $reader->close();
            libxml_clear_errors();
            return ['status' => 'error', 'msg' => 'Błąd inicjalizacji parsera XMLReader dla ABRO (LIGHT).'];
        }
        ...
        while ($reader->read()) {
            ...
            // Uwaga: w niektórych środowiskach simplexml_import_dom(XMLReader::expand())
            // potrafi zwracać NULL (nie false), co generuje warningi przy dostępie do pól.
            // Dla stabilności bierzemy outerXML i parsujemy go bezpośrednio przez SimpleXML.
            $outerXml = (string)$reader->readOuterXML();
            if ($outerXml === '') {
                continue;
            }

            $offer = @simplexml_load_string($outerXml);
            if ($offer === false || !($offer instanceof SimpleXMLElement)) {
                continue;
            }
            ...
            $targetStock = ($packQty > 0) ? (int)floor($rawStock / $packQty) : (int)floor($rawStock);
            if ($targetStock < 0) {
                $targetStock = 0;
            }

            $conversionApplied = ($packQty > 0) ? 1 : 0;

            $batch[] = "('" . pSQL($ean) . "'," . (int)$targetStock . "," . pSQL(self::formatDecimal($rawStock)) . "," . (int)$packQty . "," . (int)$conversionApplied . ")";
            $seen++;

            if (count($batch) >= 600) {
                $sql = $insertBase . implode(',', $batch) . " ON DUPLICATE KEY UPDATE
                    target_stock=VALUES(target_stock),
                    raw_stock_szt=VALUES(raw_stock_szt),
                    pack_qty=VALUES(pack_qty),
                    conversion_applied=VALUES(conversion_applied)";
                $db->execute($sql);
                $batch = [];
            }
        }
        ...
        $tmpCount = (int)$db->getValue("SELECT COUNT(*) FROM `" . bqSQL($tmpName) . "`");

        // Jeśli nie odczytaliśmy żadnego EAN, nie udawajmy sukcesu – to zwykle oznacza
        // zmianę struktury feedu albo błąd pobrania.
        if ((int)$seen === 0 || (int)$tmpCount === 0) {
            $sample = substr($xmlContent, 0, 200);
            $sample = preg_replace('/\s+/', ' ', (string)$sample);
            return [
                'status' => 'error',
                'msg' => 'ABRO LIGHT: Nie odczytano danych stanów (EAN=0). ' .
                    'Znaleziono węzłów <o>: ' . (int)$offersSeen . '. ' .
                    'Rozmiar odpowiedzi: ' . (int)strlen($xmlContent) . ' bajtów. ' .
                    'Początek odpowiedzi: ' . $sample,
            ];
        }

        // Aktualizacja finalnej tabeli RAW (bez DROP/CREATE)
        // Uwaga: extra kolumny mogą nie istnieć, więc update robimy ostrożnie.
        $setParts = [
            "t.`ilosc` = s.`target_stock`",
            "t.`stan_magazynowy_live` = s.`target_stock`",
            "t.`NaStanie` = IF(s.`target_stock` >= 1, 'True', 'False')",
            "t.`data_aktualizacji` = '" . pSQL($now) . "'",
        ];

        // Kolumny rozszerzone (jeśli istnieją)
        $columnsToTry = [
            'source_stock_szt' => "t.`source_stock_szt` = s.`raw_stock_szt`",
            'source_multiplier_w_opakowaniu' => "t.`source_multiplier_w_opakowaniu` = s.`pack_qty`",
            'conversion_applied' => "t.`conversion_applied` = s.`conversion_applied`",
        ];

        foreach ($columnsToTry as $col => $expr) {
            $colExists = (bool)$db->getValue(
                "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='" . pSQL($targetTableName) . "' AND COLUMN_NAME='" . pSQL($col) . "'"
            );
            if ($colExists) {
                $setParts[] = $expr;
            }
        ...
        $updateSql = "UPDATE `" . bqSQL($targetTableName) . "` t
            INNER JOIN `" . bqSQL($tmpName) . "` s ON s.`kod_kreskowy` = t.`kod_kreskowy`
            SET " . implode(",\n                ", $setParts);
        ...
        return [
            'status' => 'success',
            'msg' => 'ABRO LIGHT OK. Węzłów <o>: ' . (int)$offersSeen . ', wstawiono EAN: ' . (int)$seen . ', unikalnych EAN: ' . (int)$tmpCount . '. Zaktualizowano stany w azada_raw_abro.',
        ];
    }

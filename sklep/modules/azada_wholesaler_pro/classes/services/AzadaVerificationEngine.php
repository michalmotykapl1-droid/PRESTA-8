...
class AzadaVerificationEngine
{
    ...
    public static function ensureDatabase()
    {
        ...
        $tableAnalysis = _DB_PREFIX_ . 'azada_wholesaler_pro_analysis';
        $tableDiff = _DB_PREFIX_ . 'azada_wholesaler_pro_analysis_diff';

        // Tworzenie (NOWA STRUKTURA). Uwaga: jeśli tabela istnieje w starej wersji,
        // CREATE TABLE IF NOT EXISTS jej nie zmieni — dlatego poniżej jest sekcja migracji.
        $db->execute('CREATE TABLE IF NOT EXISTS `' . $tableAnalysis . '` (
        ...
        $db->execute('CREATE TABLE IF NOT EXISTS `' . $tableDiff . '` (
        ...
        // --- MIGRACJA / UPEWNIENIE KOLUMN (analysis) ---
        // Na czystej instalacji tworzy nową tabelę. Na starszych instalacjach: dopina brakujące kolumny.
        $analysisColumns = [
            // NOWE KOLUMNY WYMAGANE PRZEZ AzadaAnalysis
            'id_invoice_file' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `id_invoice_file` int(11) NOT NULL DEFAULT 0 AFTER `id_wholesaler`",
            'id_order_file' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `id_order_file` int(11) DEFAULT NULL AFTER `id_invoice_file`",
            'doc_number_order' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `doc_number_order` TEXT DEFAULT NULL AFTER `doc_number_invoice`",
            'status' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `status` varchar(20) DEFAULT 'NEW' AFTER `doc_number_order`",
            'total_diff_net' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `total_diff_net` decimal(20,2) DEFAULT 0.00 AFTER `status`",
            'items_match_count' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `items_match_count` int(11) DEFAULT 0 AFTER `total_diff_net`",
            'items_error_count' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `items_error_count` int(11) DEFAULT 0 AFTER `items_match_count`",
            // Dodajemy jako NULL, żeby ALTER nie wywalił się na tabeli z istniejącymi wierszami; potem uzupełniamy i robimy NOT NULL
            'date_analyzed' => "ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD COLUMN `date_analyzed` datetime NULL AFTER `items_error_count`",
        ];

        foreach ($analysisColumns as $colName => $alterSql) {
            try {
                $exists = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableAnalysis) . "` LIKE '" . pSQL($colName) . "'");
                if (empty($exists)) {
                    $db->execute($alterSql);
                }
            } catch (Exception $e) {
                // ignorujemy, aby nie blokować BO (różne tryby SQL / uprawnienia)
            }
        }

        // Dopilnowanie typów
        try { $db->execute("ALTER TABLE `" . bqSQL($tableAnalysis) . "` MODIFY `doc_number_order` TEXT"); } catch (Exception $e) {}
        try { $db->execute("ALTER TABLE `" . bqSQL($tableAnalysis) . "` MODIFY `total_diff_net` decimal(20,2) DEFAULT 0.00"); } catch (Exception $e) {}

        // Migracja danych (jeśli przyszliśmy ze starej struktury)
        try {
            $hasDateAdd = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableAnalysis) . "` LIKE 'date_add'");
            if (!empty($hasDateAdd)) {
                $db->execute("UPDATE `" . bqSQL($tableAnalysis) . "` SET `date_analyzed` = IFNULL(`date_analyzed`, `date_add`) WHERE `date_analyzed` IS NULL");
            } else {
                $db->execute("UPDATE `" . bqSQL($tableAnalysis) . "` SET `date_analyzed` = IFNULL(`date_analyzed`, NOW()) WHERE `date_analyzed` IS NULL");
            }
        } catch (Exception $e) {}

        // is_ok (stara kolumna) -> status (nowa)
        try {
            $hasIsOk = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableAnalysis) . "` LIKE 'is_ok'");
            if (!empty($hasIsOk)) {
                $db->execute("UPDATE `" . bqSQL($tableAnalysis) . "` SET `status` = CASE WHEN `is_ok` = 1 THEN 'OK' ELSE 'MISMATCH' END WHERE (`status` IS NULL OR `status` = '' OR `status` = 'NEW')");
            }
        } catch (Exception $e) {}

        // source_orders (stara kolumna) -> doc_number_order (nowa)
        try {
            $hasSourceOrders = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableAnalysis) . "` LIKE 'source_orders'");
            $hasDocOrder = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableAnalysis) . "` LIKE 'doc_number_order'");
            if (!empty($hasSourceOrders) && !empty($hasDocOrder)) {
                $db->execute("UPDATE `" . bqSQL($tableAnalysis) . "` SET `doc_number_order` = `source_orders` WHERE (`doc_number_order` IS NULL OR `doc_number_order` = '') AND `source_orders` IS NOT NULL");
            }
        } catch (Exception $e) {}

        // Spróbuj podpiąć id_invoice_file na podstawie numeru faktury + hurtowni (dla starych rekordów)
        try {
            $db->execute(
                "UPDATE `" . bqSQL($tableAnalysis) . "` a\n" .
                "INNER JOIN `" . bqSQL(_DB_PREFIX_ . "azada_wholesaler_pro_invoice_files") . "` i\n" .
                "    ON (i.doc_number = a.doc_number_invoice AND i.id_wholesaler = a.id_wholesaler)\n" .
                "SET a.id_invoice_file = i.id_invoice\n" .
                "WHERE (a.id_invoice_file IS NULL OR a.id_invoice_file = 0)"
            );
        } catch (Exception $e) {}

        // Date analyzed final: NOT NULL (jeśli się da)
        try { $db->execute("ALTER TABLE `" . bqSQL($tableAnalysis) . "` MODIFY `date_analyzed` datetime NOT NULL"); } catch (Exception $e) {}

        // Index id_invoice_file (dla starych tabel)
        try { $db->execute("ALTER TABLE `" . bqSQL($tableAnalysis) . "` ADD INDEX `idx_invoice_file` (`id_invoice_file`)"); } catch (Exception $e) {}
        try {
            $cols = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableDiff) . "` LIKE 'doc_number_invoice'");
            if (empty($cols)) {
                $db->execute("ALTER TABLE `" . bqSQL($tableDiff) . "` ADD `doc_number_invoice` VARCHAR(100) DEFAULT NULL AFTER `id_analysis`");
                $db->execute("ALTER TABLE `" . bqSQL($tableDiff) . "` ADD `source_orders` TEXT DEFAULT NULL AFTER `doc_number_invoice`");
            }
        } catch (Exception $e) {}
        try {
            $colsSku = $db->executeS("SHOW COLUMNS FROM `" . bqSQL($tableDiff) . "` LIKE 'wholesaler_sku'");
            if (empty($colsSku)) {
                $db->execute("ALTER TABLE `" . bqSQL($tableDiff) . "` ADD `wholesaler_sku` VARCHAR(64) DEFAULT NULL AFTER `source_orders`");
            }
        } catch (Exception $e) {}
    }

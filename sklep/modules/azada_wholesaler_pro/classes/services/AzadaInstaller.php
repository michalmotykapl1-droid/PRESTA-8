...
class AzadaInstaller
{
    public static function installDatabase()
    {
        ...
        // 5a. Stan odczytu logów per pracownik (badge "nowe")
        // Wszystkie logi modułu (w tym CRON) są w jednej tabeli: azada_wholesaler_pro_logs.
        $sql[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'azada_wholesaler_pro_logs_seen` (
        ...
        // 10. Analiza (NOWA STRUKTURA - zgodna z AzadaAnalysis + AzadaVerificationEngine)
        // Uwaga: wcześniej tabela była tworzona w starej wersji (doc_number_invoice/source_orders/summary/is_ok/date_add).
        // To powodowało problemy na czystej instalacji (brak kolumn wymaganych przez nowy silnik weryfikacji).
        ...
        // 14. Mapowanie producentów (marka hurtowni -> producent w PrestaShop)
        $sql[] = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'azada_wholesaler_pro_manufacturer_map` (
        ...
        // Domyślne konfiguracje modułu (także token CRON).
        self::installConfigDefaults();

        return true;
    }

    /**
     * Inicjalizuje domyślne wartości konfiguracji modułu.
     *
     * @return void
     */
    public static function installConfigDefaults()
    {
    ...
    /**
     * Ensure manufacturer mapping table exists (safe on upgrades).
     */
    public static function ensureManufacturerMapTables()
    {
        $sql = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'azada_wholesaler_pro_manufacturer_map` (
            `id_manufacturer_map` int(11) NOT NULL AUTO_INCREMENT,
            `source_table` varchar(64) NOT NULL,
        ...
        $table = _DB_PREFIX_ . 'azada_wholesaler_pro_manufacturer_map';

        // Backward compatibility: add missing key column if table existed without it.
        $hasKey = (bool) Db::getInstance()->getValue(
            "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS\n             WHERE TABLE_SCHEMA = DATABASE()\n               AND TABLE_NAME='" . pSQL($table) . "'\n               AND COLUMN_NAME='source_manufacturer_key'"
        );
        if (!$hasKey) {
            Db::getInstance()->execute(
                "ALTER TABLE `" . bqSQL($table) . "`\n                 ADD COLUMN `source_manufacturer_key` varchar(255) NOT NULL DEFAULT '' AFTER `source_manufacturer`"
            );
        }

        // Fill key for old rows (best-effort)
        Db::getInstance()->execute(
            "UPDATE `" . bqSQL($table) . "`\n             SET source_manufacturer_key = LOWER(TRIM(source_manufacturer))\n             WHERE source_manufacturer_key='' OR source_manufacturer_key IS NULL"
        );
    ...
    public static function uninstallDatabase()
    {
        $tables = [
            ...
            'azada_wholesaler_pro_logs_seen',
            ...
            'azada_wholesaler_pro_manufacturer_map'
        ];

...
$capabilities = $this->getHubActionCapabilities(isset($row['raw_table_name']) ? (string)$row['raw_table_name'] : '');
    ...
    public function __construct()
    {
        ...
        // Auto-dodawanie nowych hurtowni:
        // Jeśli w bazie pojawi się nowa tabela RAW (np. azada_raw_abro),
        // moduł automatycznie dodaje ją do konfiguracji (tabela integracji).
        // Dzięki temu hurtownia od razu jest widoczna w HUB.
        $this->syncMissingIntegrationsFromRawTables();
...
function runImport(event, btn, url) {
  ...
  if (data.status == 'success') {
    ...
    /**
     * Jeśli w bazie pojawiła się nowa tabela RAW (np. azada_raw_abro),
     * a nie ma jej jeszcze w tabeli integracji, to dodaj ją automatycznie.
     */
    private function syncMissingIntegrationsFromRawTables()
    {
        $db = Db::getInstance();

        $integrationTable = _DB_PREFIX_ . 'azada_wholesaler_pro_integration';
        $exists = (int)$db->getValue(
            "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='" . pSQL($integrationTable) . "'"
        );
        if ($exists <= 0) {
            return;
        }
        ...
        $rawRows = $db->executeS("SHOW TABLES LIKE '" . $prefix . "azada_raw_%'");
        ...
        foreach ($rawRows as $row) {
            ...
            $count = (int)$db->getValue(
                'SELECT COUNT(*) FROM `' . bqSQL($integrationTable) . '` WHERE raw_table_name=\'' . pSQL($table) . '\''
            );
            if ($count > 0) {
                continue;
            }

            $name = ucfirst(str_replace('azada_raw_', '', $table));
            if ($name === '') {
                $name = $table;
            }
            ...
            $db->insert('azada_wholesaler_pro_integration', $payload, true);
        }
    }

...
class Azada_Wholesaler_Pro extends Module
{
    public function __construct()
    {
        ...
        // Auto-rejestracja hooka do badge logów CRON (żeby nie wymagać reinstalacji modułu)
        if ($this->id && !$this->isRegisteredInHook('displayBackOfficeHeader')) {
            $this->registerHook('displayBackOfficeHeader');
        }

        // Auto-doinstalowanie brakującej zakładki Logi (CRON) po aktualizacji
        // (wykona się tylko w back office, gdy jest zalogowany pracownik)
        if ($this->id && isset($this->context) && isset($this->context->employee) && (int)$this->context->employee->id > 0) {
            try {
                if ((int)Tab::getIdFromClassName('AdminAzadaCronLogs') <= 0) {
    ...
    /**
     * Badge "nowe logi CRON" w menu + dołączenie JS.
     */
    public function hookDisplayBackOfficeHeader($params)
    {
        ...
        if (method_exists($this->context->controller, 'addJS')) {
            $this->context->controller->addJS($this->_path . 'views/js/cron_log_badge.js');
        }

        // AJAX URL do pobrania liczby nowych logów
        $baseLink = $this->context->link->getAdminLink('AdminAzadaCronLogs', true);
        $ajaxUrl = $baseLink . (strpos($baseLink, '?') === false ? '?' : '&') . 'ajax=1&action=GetNewCount';

        if (class_exists('Media')) {
            Media::addJsDef([
                'AZADA_CRON_LOG_BADGE_URL' => $ajaxUrl,
                'AZADA_CRON_LOG_TAB_CLASS' => 'AdminAzadaCronLogs',
            ]);
        }
    }

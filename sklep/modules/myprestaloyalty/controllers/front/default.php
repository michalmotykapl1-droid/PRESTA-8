...
class myprestaloyaltyDefaultModuleFrontController extends ModuleFrontController
{
    ...
    public function processTransformPoints()
    {
        ...
        if ($customer_points > 0) {
            ...
            // --- ZMIANA: Wyłączamy automatyczne wyświetlanie w standardowym polu PrestaShop ---
            ...
            /* --- POPRAWKA: Data ważności pobierana z ustawień modułu --- */
            $validity_period = (int)Configuration::get('PS_LOYALTY_VALIDITY_PERIOD');
            if ($validity_period > 0) {
                $cart_rule->date_to = date('Y-m-d H:i:s', strtotime($cart_rule->date_from . ' +' . $validity_period . ' days'));
            } else {
                $cart_rule->date_to = date('Y-m-d H:i:s', strtotime($cart_rule->date_from . ' +180 days'));
            }
            /* ----------------------------------------------------------- */

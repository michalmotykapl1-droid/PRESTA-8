            ...
            if ($forceUpdateAll) {
                $upd = [
                    ...
                    'type_name' => $typeName ?: null,
	                    ...
	                    'offer_name' => $offerNameRaw !== '' ? $offerName : null,
	                    'order_id' => $orderIdRaw !== '' ? $orderId : null,
                    'payment_id' => ($paymentIdRaw !== '' ? $paymentId : ((string)($existsRow['payment_id'] ?? '') !== '' ? pSQL((string)$existsRow['payment_id']) : null)),
                    'value_amount' => (float)$valAmount,
                    'value_currency' => $valCurrency ?: 'PLN',
                    'balance_amount' => $balAmount,
                    'balance_currency' => $balCurrency ?: null,
                ...
                if ($isEmpty($existsRow['payment_id'] ?? null) && $paymentIdRaw !== '') {
                    $upd['payment_id'] = $paymentId;
                }
...
/**
 * Masowe dopięcie order_id (checkoutFormId) do wpisów billing na podstawie payment_id.
 *
 * Użyteczne gdy Allegro zwróciło billing-entries bez order_id, ale z payment_id.
 * Robimy JOIN do lokalnego cache płatności (allegropro_order_payment) i zamówień (allegropro_order)
 * aby nie pomylić kont.
 *
 * Zwraca liczbę zaktualizowanych rekordów.
 */
public function attachMissingOrderIdsFromPayments(int $accountId, string $dateFrom, string $dateTo): int
{
    if ($accountId <= 0) {
        return 0;
    }
    ...
    $sql = "UPDATE `{$p}allegropro_billing_entry` b\n"
        . "INNER JOIN `{$p}allegropro_order_payment` op ON op.payment_id = b.payment_id\n"
        . "INNER JOIN `{$p}allegropro_order` o ON o.checkout_form_id = op.checkout_form_id AND o.id_allegropro_account=" . (int)$accountId . "\n"
        . "SET b.order_id = op.checkout_form_id, b.updated_at = '{$now}'\n"

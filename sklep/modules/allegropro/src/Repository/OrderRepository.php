...
class OrderRepository
{
    ...
    /**
     * Kompatybilność z innymi repozytoriami (PaymentOperationRepository/BillingEntryRepository).
     *
     * W starszych wersjach modułu repozytorium zamówień nie posiadało metody ensureSchema().
     * W nowszych widokach (np. "Rozliczenie") używamy jej jako lekkiego sprawdzenia
     * czy tabela cache zamówień istnieje.
     *
     * Uwaga: ta metoda NIE wykonuje migracji schematu – tylko weryfikuje istnienie tabeli.
     */
    public function ensureSchema(): bool
    {
        $table = _DB_PREFIX_ . 'allegropro_order';
        try {
...
'
    ...
    /**
     * Zapisuje pełne dane zamówienia do cache.
     *
     * Backward-compat:
     * - starsze wywołania: saveFullOrder(array $data) (gdzie $data zawiera account_id)
     * - nowsze wywołania (np. odświeżanie statusów po synchronizacji przepływów):
     *   saveFullOrder(int $accountId, array $checkoutFormJson)
     *
     * @param mixed $data
     * @param array|null $checkoutFormJson
     */
    public function saveFullOrder($data, ?array $checkoutFormJson = null)
    {
        // Obsługa wywołania: saveFullOrder($accountId, $checkoutFormJson)
        if (is_int($data) && is_array($checkoutFormJson)) {
            $checkoutFormJson['account_id'] = (int)$data;
            $data = $checkoutFormJson;
        }

        if (!is_array($data)) {
            throw new \InvalidArgumentException('saveFullOrder expects array $data or (int $accountId, array $checkoutFormJson).');
        }

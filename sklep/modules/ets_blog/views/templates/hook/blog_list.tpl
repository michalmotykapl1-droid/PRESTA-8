...
<div class="ets_blog_layout_{$blog_layout|escape:'html':'UTF-8'} ets-blog-wrapper ets-blog-wrapper-blog-list{if isset($ets_blog_config.ETS_BLOG_AUTO_LOAD) &&$ets_blog_config.ETS_BLOG_AUTO_LOAD} loadmore{/if} {if $blog_latest}ets-page-latest{elseif $blog_category}ets-page-category{elseif $blog_search}ets-page-search{elseif $author}ets-page-author{else}ets-page-home{/if}">
    ...
    {elseif $blog_search}
        ...
        {* POCZĄTEK MODYFIKACJI - Wyświetlanie znalezionych kategorii z licznikiem *}
        {if isset($blog_found_categories) && $blog_found_categories}
            <div class="ets_blog_found_categories">
                <span class="found-in-label">{l s='Znaleziono w kategoriach' mod='ets_blog'}:</span>
                {foreach from=$blog_found_categories item='cat' name=catlist}
                    <a href="{$cat.link|escape:'html':'UTF-8'}">
                        {$cat.title|escape:'html':'UTF-8'}
                        {if isset($cat.post_count)}<span class="category-post-count">({$cat.post_count|intval})</span>{/if}
                    </a>
                {/foreach}
            </div>
        {/if}
                ...
                {foreach from=$blog_posts item='post'}            
                    <li>                         
                        <div class="post-wrapper">
                            {if $is_main_page && $first_post && ($blog_layout == 'large_list' || $blog_layout == 'large_grid')}
                                {if $post.image}
                                    <a class="ets_item_img{if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD)&& $ets_blog_config.ETS_BLOG_LAZY_LOAD} ets_item_img_ladyload{/if}" href="{$post.link|escape:'html':'UTF-8'}">
                                        {* --- MODYFIKACJA WEBP START (Main Image) --- *}
                                        {assign var="webp_img" value=$post.image|replace:'.jpg':'.webp'|replace:'.png':'.webp'|replace:'.jpeg':'.webp'}
                                        <img title="{$post.title|escape:'html':'UTF-8'}" src="{if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD) && $ets_blog_config.ETS_BLOG_LAZY_LOAD}{$link->getMediaLink("`$smarty.const._MODULE_DIR_`ets_blog/views/img/bg-grey.png")|escape:'html':'UTF-8'}{else}{$webp_img|escape:'html':'UTF-8'}{/if}" alt="{$post.title|escape:'html':'UTF-8'}" {if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD)&& $ets_blog_config.ETS_BLOG_LAZY_LOAD} data-original="{$webp_img|escape:'html':'UTF-8'}" class="lazyload"{/if} />
                                ...
                                {elseif $post.thumb}
                                    <a class="ets_item_img{if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD)&& $ets_blog_config.ETS_BLOG_LAZY_LOAD} ets_item_img_ladyload{/if}" href="{$post.link|escape:'html':'UTF-8'}">
                                        {* --- MODYFIKACJA WEBP START (Thumb Image - Large Layout) --- *}
                                        {assign var="webp_thumb" value=$post.thumb|replace:'.jpg':'.webp'|replace:'.png':'.webp'|replace:'.jpeg':'.webp'}
                                        <img title="{$post.title|escape:'html':'UTF-8'}" src="{if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD) && $ets_blog_config.ETS_BLOG_LAZY_LOAD}{$link->getMediaLink("`$smarty.const._MODULE_DIR_`ets_blog/views/img/bg-grey.png")|escape:'html':'UTF-8'}{else}{$webp_thumb|escape:'html':'UTF-8'}{/if}" alt="{$post.title|escape:'html':'UTF-8'}" {if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD)&& $ets_blog_config.ETS_BLOG_LAZY_LOAD} data-original="{$webp_thumb|escape:'html':'UTF-8'}" class="lazyload"{/if} />
                                        {* --- MODYFIKACJA WEBP KONIEC --- *}
                            ...
                            {elseif $post.thumb}
                                <a class="ets_item_img{if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD)&& $ets_blog_config.ETS_BLOG_LAZY_LOAD} ets_item_img_ladyload{/if}" href="{$post.link|escape:'html':'UTF-8'}">
                                    {* --- MODYFIKACJA WEBP START (Thumb Image - Standard Layout) --- *}
                                    {assign var="webp_thumb" value=$post.thumb|replace:'.jpg':'.webp'|replace:'.png':'.webp'|replace:'.jpeg':'.webp'}
                                    <img title="{$post.title|escape:'html':'UTF-8'}" src="{if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD) && $ets_blog_config.ETS_BLOG_LAZY_LOAD}{$link->getMediaLink("`$smarty.const._MODULE_DIR_`ets_blog/views/img/bg-grey.png")|escape:'html':'UTF-8'}{else}{$webp_thumb|escape:'html':'UTF-8'}{/if}" alt="{$post.title|escape:'html':'UTF-8'}" {if isset($ets_blog_config.ETS_BLOG_LAZY_LOAD)&& $ets_blog_config.ETS_BLOG_LAZY_LOAD} data-original="{$webp_thumb|escape:'html':'UTF-8'}" class="lazyload"{/if} />
                                    {* --- MODYFIKACJA WEBP KONIEC --- *}

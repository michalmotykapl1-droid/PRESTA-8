...
{strip}
{if count($groups) > 0}
{* Dodano klasę 'native-variants-container' dla odróżnienia od modułów *}
<div class="product-variants native-variants-container">
  {foreach from=$groups key=id_attribute_group item=group}
    {if !empty($group.attributes)}
    <div class="clearfix product-variants-item">
      <span class="control-label">{$group.name} : </span>
      {if $group.group_type == 'select'}
        {* Dodano style inline z-index, aby select był zawsze na wierzchu *}
